...
class MTBert:
    ...
    def __str__(self):
        return f"<MTBert({self.device}): {self.model_path}>"
    ...
    def summarize(input, rec1, rec2, spidx, spans, sppols, isbatch):
        ...
        if not isbatch:
            return {'text': input, 
                    ...
                    'span_pols': sppols[0],                    
                    'token_probs':rec1['probs']
                    }
        ListofDicts = []

        for i in range(len(input)):
            Dict = {'text': input[i], 'seq_polarity': pols[i], 'pol_probs':rec2['probs'][i], 'token_logits': rec1['logits'][i],\
    ...
    def predict(self, input:str)->Dict[str, any]:
        ...
        t2records = {'pol':[], 'probs':[]}
        X = self.tokenizer(X, truncation=True, is_split_into_words=True,
